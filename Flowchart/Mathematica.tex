...
\begin{figure}[!ht]
	...
	\begin{tikzpicture}
		...
		\node (input) [input, below of = start, yshift = -1cm] {Input $n$.};
		\note (setup) [process, below of = input, yshift = -1cm] {$sum = 0$, $i = 1$};
		...
		\draw [arrow] (check) -- node[anchor=north] {no} (increase);
		\draw [arrow] (check) -- node[anchor=east] {yes} (return);
