...
\begin{figure}[!ht]
	...
	\begin{tikzpicture}
		...
		%\node (add) [process, right of = div, xshift = 3cm] {$count:= count + 1$};
		\node (return) [process, below of = check, yshift = -1cm] {Return $count$};
		\node (increase) [process, left of = div, xshift = -3cm] {$i:=i+1$};
		\node (stop) [startstop, below of = return, yshift = -1cm] {Stop};
		%\draw [arrow] (start) -- (input);
		%\draw [arrow] (input) -- (setup);
...
Modify the algorithm above to determine if an integer $n$ is prime (prime numbers are numbers greater than 1 that are only divisible by 1 and themselves).
\end{document}
