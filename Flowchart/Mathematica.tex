...
\begin{problem}
	Create an algorithm that counts the number of divisors of $n$ for any integer $n>0$. 
	\begin{hint}
	You may need two decisions to solve this problem. Note, the second hint for this problem is the solution.
	\end{hint}
	...
	\begin{image}
		\begin{tikzpicture}
			...
			\node (check) [decision, below of = setup, yshift = -1cm] {$n\%i=0$};
			\node (div) [process, right of = check, xshift = 3cm] {$count:=count+1$};
			\node (end) [decision, below of = check, yshift = -1cm] {$i=n$};
			\node (increase) [process, left of = check, xshift = -3cm] {$i:=i+1$};
			\node (return) [process, below of = end, yshift = -1cm] {Return $count$};
			\node (stop) [startstop, below of = return, yshift = -1cm] {Stop};
			\draw [arrow] (start) -- (input);
			\draw [arrow] (input) -- (setup);
			\draw [arrow] (setup) -- (check);
			\draw [arrow] (check) -- node[anchor=north] {true} (div);
			\draw [arrow] (check) -- node[anchor=east] {false} (end);
			\draw [arrow] (end) -- node[anchor=west] {false} (increase);
			\draw [arrow] (end) -- node[anchor=east] {true} (return);
			\draw [arrow] (increase) -- (check);
			\draw [arrow] (return) -- (stop);
		\end{tikzpicture}
		\caption{Example of an algorithm that computes the number of divisors of $n$.}
	\end{image}
	\end{hint}
\end{problem}
