...
Another example of recursion is given by the \link[Fibonacci sequence]{https://plus.maths.org/content/fibonacci-sequence-brief-introduction}. We define $f_n$, the $n$th Fibonacci number of $n\geq 0$ as $$f_n=\begin{cases} 1 & \text{if $n=0,1$}\\ f_{n-1}+f_{n-2} & \text{otherwise.}\end{cases}$$ Computing recursively, we see that this definition yields the sequence $1,1,2,3,5,8,\cdots$. That is, for every $n\geq 2$, $f_n$ is the sum of the previous two terms in the sequence.
